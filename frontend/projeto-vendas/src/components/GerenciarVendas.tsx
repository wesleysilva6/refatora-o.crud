    ...
    <div className="container-fluid">
        ...
        <div className="card-body">
          <div className="row g-3">
            <div className="col-sm-12 col-md-3">
              ...
              <input type="date" value={de} onChange={(e) => setDe(e.target.value)} className="form-control" />
            </div>
            <div className="col-sm-12 col-md-3">
              <label className="form-label">Até</label>
              <input type="date" value={ate} onChange={(e) => setAte(e.target.value)} className="form-control" />
            </div>
            <div className="col-sm-12 col-md-4">
              <label className="form-label">Buscar (cliente/telefone)</label>
              <input
                value={q}
                onChange={(e) => setQ(e.target.value)}
                className="form-control"
                placeholder="ex: Maria, 11 9xxxx-xxxx"
              />
            </div>
      ...
      <div className="row g-3 mb-3">
        <div className="col-12 col-md-6 col-xl-3">
          <div className="card">
            <div className="card-body">
              <div className="text-muted small">Vendas listadas</div>
            ...
            <>
                  ...
                  <tbody>
                    {(resp?.data ?? []).length === 0 ? (
                      ...
                      resp!.data.map((v) => (
                        <tr key={v.id}>
                          <td>{v.id}</td>
                          <td>{fmtDateTime(v.realizada_em)}</td>
                          <td>{v.cliente}</td>
                          <td>{v.funcionario?.nome ?? "—"}</td>
                          <td>{v.telefone ?? "—"}</td>
                          <td className="text-end">{fmtBRL(v.total)}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
              ...
              <div className="d-flex justify-content-between align-items-center mt-2">
                ...
                <div className="d-flex align-items-center gap-2">
                  ...
                  <button
                    className="btn btn-sm btn-outline-secondary"
                    disabled={!resp || page >= (resp?.last_page ?? 1)}
